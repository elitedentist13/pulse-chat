...
async function api(path, expect = 200) {
  ...
  return { url, status: response.status, headers: response.headers, text, ms, ok: response.status === expect }
}
...
async function main() {
  ...
  record("api", "GET /favicon.ico", favicon.status === 200 || favicon.status === 404, `${favicon.status}`)
  ...
  record("cdp", "seed has care, stories, pages, talents", Boolean(seed && seed.care >= 6 && seed.stories >= 1 && seed.entries >= 8 && seed.talents >= 3))

  // --- Smoke / client shell ---
  record(
    "smoke",
    "nav has Daybook Porch Notes",
    await page.$eval("[data-app-nav]", (el) =>
      /Daybook/.test(el.innerText) && /Porch/.test(el.innerText) && /Notes/.test(el.innerText)
    )
  )
  record(
    "smoke",
    "locale switch EN 繁 简",
    await page.$$eval("[data-locale-switch] [data-locale]", (els) =>
      els.map((el) => el.getAttribute("data-locale")).join(",")
    ).then((ids) => ids === "en,zh-Hant,zh-Hans")
  )

  // --- Spot: profile ---
  await page.click("[data-pet-tab='profile']")
  await page.waitForSelector("[data-profile-desk]")
  const profile = await page.$eval("[data-profile-desk]", (el) => el.innerText)
  record("spot", "profile shows Juniper dossier fields", /Juniper/.test(profile) && /Date of birth|Nickname|Traits|Medical remarks|Favourite food/.test(profile))

  await page.click("[data-locale='zh-Hant']")
  ...
  record("spot", "Traditional Chinese profile labels", /出生日期/.test(hant) && /性情/.test(hant) && /醫療備註/.test(hant))

  await page.click("[data-locale='zh-Hans']")
  ...
  record("spot", "Simplified Chinese nav", /日记本/.test(hansNav) && /门廊/.test(hansNav) && /便笺/.test(hansNav))
  await page.click("[data-locale='en']")
  await page.waitForFunction(() => document.documentElement.dataset.locale === "en")

  // --- Spot: care strip + lightbox ---
  await page.click("[data-pet-tab='care']")
  ...
  record(
    "spot",
    "care subtabs present",
    ["prevent", "bowl", "visit", "blood", "receipt", "prescription", "groom", "food"].every((id) =>
      subtabs.includes(id)
    ),
    subtabs.join(",")
  )
  ...
  record("spot", "visit strip has dated captioned slip", /Annual \+ limp check/.test(visit) && /2026/.test(visit))

  await page.click("[data-care-slide-open]")
  await page.waitForSelector("[data-care-lightbox]")
  record("client", "lightbox opens on slip click", true)
  const cap = await page.$("[data-slide-caption]")
  await cap.click({ clickCount: 3 })
  await cap.type("Live-test visit")
  await page.click("[data-slide-save]")
  ...
  record("client", "lightbox save writes caption back to strip", visitAfter.includes("Live-test visit"))

  await page.click("[data-care-subtab-btn='groom']")
  await page.waitForSelector("[data-care-strip='groom']")
  const groomCount = await page.$$eval("[data-care-slide]", (els) => els.length)
  record("spot", "groom strip has two slips", groomCount >= 2, String(groomCount))
  await page.click("[data-care-subtab-btn='prevent']")
  await page.waitForSelector("[data-reminders]")
  record(
    "spot",
    "preventatives tab lists ticks reminder",
    await page.$eval("[data-reminders]", (el) => /Ticks/.test(el.innerText))
  )

  // --- Spot: talent ---
  await page.click("[data-pet-tab='talent']")
  await page.waitForSelector("[data-talent-desk]")
  record(
    "spot",
    "talent desk lists Stand and Give a hand",
    await page.$eval("[data-talent-desk]", (el) => /Stand/.test(el.innerText) && /Give a hand/.test(el.innerText))
  )

  // --- Client: daybook + video limits ---
  await page.click("[data-pet-tab='pages']")
  ...
  record("client", "20s clip rejected at 15s cap", /15 seconds/.test(tooLong), tooLong)
  await mediaInput.uploadFile("/tmp/short-clip.mp4")
  await page.waitForSelector("[data-media-kind='video']", { timeout: 15000 })
  record("client", "4s clip accepted on the page", true)
  await page.screenshot({ path: "/opt/cursor/artifacts/live_test_day_video.png" })

  // --- Client: porch + notes ---
  await page.click("[data-surface-tab='porch']")
  await page.waitForSelector("[data-porch]")
  const porchCount = await page.$$eval("[data-porch-entry]", (els) => els.length)
  record("client", "porch lists public pages", porchCount >= 4, String(porchCount))
  await page.click("[data-surface-tab='notes']")
  await page.waitForFunction(() => document.documentElement.dataset.surface === "notes")
  record(
    "client",
    "notes table is the adjunct",
    await page.evaluate(() => /Notes/.test(document.body.innerText) && /Open|Waiting|Hall/.test(document.body.innerText))
  )
  await page.evaluate(() => {
  ...
  record("client", "hall shelves mount", Boolean(await page.$("[data-hall]")))

  // --- CDP after interactions ---
